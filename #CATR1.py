...
# --- macOS-safe button helper ---
def make_mac_button(parent, text, command, font=("Segoe UI", 11, "bold"), width=None):
    """Create a button that renders silver bg + blue text on macOS."""
    silver = "#b0b8c4"
    blue = "#00aaff"
    hover_silver = "#c8cfd9"
    active_silver = "#8a929c"
    ...
    def on_enter(e):
        btn.config(bg=hover_silver)
        lbl.config(bg=hover_silver)
    def on_leave(e):
        btn.config(bg=silver)
        lbl.config(bg=silver)
    def on_click(e):
        btn.config(bg=active_silver)
        lbl.config(bg=active_silver)
...
class CatR1:
    def __init__(self, root):
        ...
        # --- SEND Button (Silver BG, Blue Text — macOS safe) ---
        self.send_btn = make_mac_button(self.input_frame, "➤", self.send_click)
        self.send_btn.pack(side=tk.LEFT, padx=(0, 5), pady=8)

        # --- CLOSE Button (Silver BG, Blue Text — macOS safe) ---
        self.close_btn = make_mac_button(self.input_frame, "✕", root.destroy,
                                         font=("Segoe UI", 14, "bold"), width=3)
        self.close_btn.pack(side=tk.LEFT, padx=(0, 10), pady=8)
    ...
    def add_bubble(self, role, content):
        ...
        text_justify = "left" if role=="assistant" else "right"

        bubble = tk.Label(frame, text=content, wraplength=500, justify=text_justify,
    ...
    def send(self, event=None):
        ...
        self.add_bubble("assistant", "thinking...")
    ...
    def stream_response(self, response):
        for widget in self.chat_frame.winfo_children()[::-1]:
            if isinstance(widget, tk.Frame):
                ...
                if children and "thinking..." in children[0].cget("text"):
                    widget.destroy()
                    break

        current_text = ""
        frame = tk.Frame(self.chat_frame, bg="#202123")
        bubble = tk.Label(frame, text="", wraplength=500, justify="left",
